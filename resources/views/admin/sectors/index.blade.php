    ...
    <div x-data="{ openDeleteModal: false, deleteAction: '' }">
        <div class="mb-10 flex items-center justify-between">
            <div>
                ...
                <p class="text-mcc-slate-500 text-sm font-medium mt-1">
                    {{ __('Manage business areas and industrial sectors') }}</p>
            </div>
                            ...
                            <tr class="hover:bg-mcc-blue-50/30 transition-colors group">
                                <td class="px-10 py-6">
                                    <div class="flex items-center">
                                        <div class="w-12 h-12 bg-mcc-slate-100 rounded-xl overflow-hidden flex-shrink-0 mr-4">
                                            @if($sector->image_url)
                                                <img src="{{ $sector->image_url }}" class="w-full h-full object-cover">
                                            @else
                                        ...
                                        <div>
                                            <div class="text-sm font-bold text-mcc-slate-900 tracking-tight">{{ $sector->name }}
                                            </div>
                                            <div
                                ...
                                <td class="px-6 py-6">
                                    <span class="text-sm font-bold text-mcc-slate-600">
                                        {{ $sector->projects_count ?? ($sector->projects ? $sector->projects->count() : 0) }}
                                        {{ __('Projects') }}
                                    </span>
                                </td>
                                <td class="px-6 py-6">
                                    <p class="text-xs font-medium text-mcc-slate-500 line-clamp-1 max-w-xs">
                                ...
                                <td class="px-10 py-6 text-right space-x-2">
                                    ...
                                    <button
                                        @click="openDeleteModal = true; deleteAction = '{{ route('admin.sectors.destroy', $sector) }}'"
        ...
        <div x-show="openDeleteModal"
            ...
            <div class="bg-white rounded-[2.5rem] w-full max-w-sm p-10 shadow-2xl overflow-hidden relative">
                <h3 class="text-xl font-black text-mcc-slate-900 uppercase tracking-tight mb-4">{{ __('Delete Sector?') }}
                </h3>
                <p class="text-mcc-slate-500 text-sm font-medium mb-8">
                    {{ __('This action is permanent and might affect many projects. Are you absolutely sure?') }}</p>
