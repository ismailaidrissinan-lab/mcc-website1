    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-mcc-slate-200 overflow-hidden">
            <div class="p-8 border-b border-mcc-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold text-mcc-slate-900">
                        {{ isset($training) ? 'Edit Program' : 'Create New Training Program' }}</h3>
                    <p class="text-sm text-mcc-slate-500 mt-1">Define the curriculum and location for this developmental
                        initiative.</p>
                </div>
            ...
            <form
                action="{{ isset($training) ? route('admin.training.update', $training) : route('admin.training.store') }}"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-bold text-mcc-slate-700 uppercase tracking-widest">Hero Image
                        (Optional)</label>
                    <div class="mt-2 flex items-center space-x-6">
                        @if(isset($training) && $training->image_url)
