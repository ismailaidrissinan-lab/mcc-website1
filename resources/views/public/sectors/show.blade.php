...
@section('content')
    ...
    <div class="relative pt-32 pb-20 bg-mcc-slate-900 overflow-hidden">
        <div class="absolute inset-0 opacity-30">
            <img src="{{ $sector->image_url }}" alt="{{ $sector->name }}" class="w-full h-full object-cover">
        </div>
        <div class="container-wide relative z-10">
            <div
                class="inline-flex items-center space-x-2 px-3 py-1 bg-mcc-gold/20 border border-mcc-gold/30 rounded-full mb-6">
                <span
                    class="text-mcc-gold text-[10px] font-bold uppercase tracking-widest">{{ __('Business Sector') }}</span>
            </div>
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-20">
                ...
                <div class="lg:col-span-8 space-y-16">
                    <div class="space-y-8" reveal>
                        <h2 class="text-3xl font-bold text-mcc-slate-900 tracking-tight">
                            {{ __('Strategic Expertise & Solutions') }}
                        </h2>
                    ...
                    <div class="space-y-10" reveal>
                        <div class="flex items-center justify-between border-b border-mcc-slate-100 pb-6">
                            ...
                            <span
                                class="text-xs font-bold text-mcc-gold uppercase tracking-widest">{{ $sector->projects->count() }}
                                {{ __('Projects') }}</span>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                @foreach($sector->projects as $project)
                                    <a href="{{ route('projects.show', $project->slug) }}"
                                        class="group block bg-white rounded-3xl overflow-hidden border border-mcc-slate-100 shadow-sm transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
                                            ...
                                            @else
                                                <div class="w-full h-full flex items-center justify-center bg-mcc-slate-50">
                                                    <img src="{{ asset('images/mcc-logo.png') }}" alt="MCC Ltd"
                                                        class="w-24 h-auto opacity-20">
                                                </div>
                                            @endif
                                            <div
                                                class="absolute inset-0 bg-gradient-to-t from-mcc-slate-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                            </div>
                                        </div>
                                        <div class="p-8">
                                            <div
                                                class="flex items-center justify-between mb-3 text-xs font-bold text-mcc-gold uppercase tracking-widest">
                                                <span>{{ $project->location }}</span>
                                                <span>{{ $project->completion_date ? $project->completion_date->format('Y') : '' }}</span>
                                            </div>
                                            <h4
                                                class="text-xl font-bold text-mcc-slate-900 group-hover:text-mcc-blue-600 transition-colors duration-300">
                                                {{ $project->title }}
                                            </h4>
                                            <div
                                                class="mt-6 flex items-center text-sm font-bold text-mcc-blue-600 transition-all duration-300 opacity-0 transform translate-x-[-10px] group-hover:opacity-100 group-hover:translate-x-0">
                                                {{ __('View Case Study') }}
                        ...
                        @else
                            <div
                                class="p-20 bg-mcc-slate-50 rounded-[3rem] text-center border-2 border-dashed border-mcc-slate-100">
                                <div
                                    class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-mcc-slate-300 mx-auto mb-6 shadow-sm">
                ...
                <div class="lg:col-span-4 mt-16 lg:mt-0">
                    <div class="sticky top-32 space-y-8" reveal>
                        ...
                        <div class="bg-mcc-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                            ...
                            <h3 class="text-mcc-gold text-[10px] font-bold uppercase tracking-[0.2em] mb-10">
                                {{ __('Our Solutions') }}
                            </h3>
                            <ul class="space-y-6">
                                <li class="flex items-center space-x-4">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-mcc-gold">
                                ...
                                <li class="flex items-center space-x-4">
                                    ...
                                    <span
                                        class="text-sm font-medium tracking-wide">{{ __('Sustainable Maintenance') }}</span>
                                </li>
                            </ul>

                            <a href="{{ route('contact') }}"
                                class="btn-corporate w-full bg-white text-mcc-slate-900 border-white hover:bg-mcc-gold hover:border-mcc-gold justify-center mt-12 py-5">
