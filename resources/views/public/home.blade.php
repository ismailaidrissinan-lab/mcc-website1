...
@section('content')
    ...
    <div class="relative h-screen min-h-[700px] flex items-center overflow-hidden bg-mcc-slate-900">
        ...
        <div class="container-wide relative z-10 pt-20">
            <div class="max-w-3xl space-y-8">
                <div
                    class="inline-flex items-center space-x-2 px-3 py-1 bg-mcc-gold/20 border border-mcc-gold/30 rounded-full">
                    <span class="w-2 h-2 rounded-full bg-mcc-gold animate-pulse"></span>
                    <span
                        class="text-mcc-gold text-[10px] font-bold uppercase tracking-widest">{{ __('Engineering Excellence') }}</span>
                </div>
                ...
                <p class="text-xl text-mcc-slate-300 font-light max-w-2xl leading-relaxed">
                    {{ __('Engineering Excellence Across Africa and Asia.') }}
                    {{ __('We leverage international expertise and local knowledge to deliver projects that power communities and drive economic growth.') }}
                </p>

                <div class="flex flex-col sm:flex-row gap-5 pt-4">
                    <a href="{{ route('projects.index') }}"
                        class="btn-corporate bg-mcc-blue-600 text-white hover:bg-mcc-blue-700 shadow-mcc-blue-600/30">
                    ...
                    <a href="{{ route('about') }}"
                        class="btn-corporate bg-white/10 backdrop-blur-sm text-white border border-white/20 hover:bg-white/20">
                        {{ __('Learn More') }}
                    </a>
                </div>
            </div>
    ...
    <section class="section-padding bg-mcc-slate-50 relative overflow-hidden" reveal>
        ...
        <div class="container-wide relative z-10">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8">
                <div
                    class="group p-10 bg-white rounded-3xl shadow-xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 border border-mcc-slate-100 reveal-delay-100">
                    <div
                        class="w-16 h-16 bg-mcc-blue-50 rounded-2xl flex items-center justify-center text-mcc-blue-600 mb-8 transform transition-transform duration-500 group-hover:scale-110">
                ...
                <div
                    class="group p-10 bg-mcc-blue-900 rounded-3xl shadow-2xl transition-all duration-500 hover:scale-[1.02] relative overflow-hidden reveal-delay-200">
                    <div
                        class="absolute -top-10 -right-10 w-40 h-40 bg-white/5 rounded-full scale-150 transition-transform duration-700 group-hover:scale-[2]">
                    </div>
                    <div
                        class="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-mcc-gold mb-8 transform transition-transform duration-500 group-hover:rotate-12">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div
                    class="group p-10 bg-white rounded-3xl shadow-xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 border border-mcc-slate-100 reveal-delay-300">
                    <div
                        class="w-16 h-16 bg-mcc-blue-50 rounded-2xl flex items-center justify-center text-mcc-blue-600 mb-8 transform transition-transform duration-500 group-hover:scale-110">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <section class="section-padding bg-mcc-slate-50 overflow-hidden">
        <div class="container-wide">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6" reveal>
                <div class="max-w-2xl">
                    <h2 class="text-mcc-gold font-bold text-xs uppercase tracking-[0.2em] mb-4">{{ __('Core Sectors') }}
                    </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                @foreach(\App\Models\Sector::take(4)->get() as $sector)
                    <a href="{{ route('sectors.show', $sector->slug) }}"
                        class="group relative aspect-[4/5] rounded-[2.5rem] overflow-hidden shadow-xl transition-all duration-700 hover:shadow-2xl hover:-translate-y-4"
                        reveal>
                        <img src="{{ $sector->image_url }}" alt="{{ $sector->name }}"
                            class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110">
                        <div class="absolute inset-0 bg-gradient-to-t from-mcc-blue-950 via-mcc-blue-950/40 to-transparent">
                        </div>
                        <div
                            class="absolute inset-x-0 bottom-0 p-10 transform translate-y-4 transition-transform duration-500 group-hover:translate-y-0">
                            <h4 class="text-2xl font-bold text-white mb-4">{{ $sector->name }}</h4>
                            <p
                                class="text-mcc-blue-100/70 text-sm line-clamp-2 font-light mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                {{ $sector->description }}
                            </p>
    ...
    <section class="section-padding bg-white overflow-hidden">
        <div class="container-wide">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6" reveal>
                <div class="max-w-2xl">
                    <h2 class="text-mcc-gold font-bold text-xs uppercase tracking-[0.2em] mb-4">{{ __('Latest Updates') }}
                    </h2>
                    <h3 class="text-4xl md:text-5xl font-bold tracking-tight text-mcc-slate-900 leading-tight">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                @foreach($latestArticles as $article)
                    <div class="group flex flex-col bg-white rounded-3xl overflow-hidden border border-mcc-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500"
                        reveal>
                        <div class="aspect-[16/9] overflow-hidden relative">
                            <img src="{{ $article->image_url }}" alt="{{ $article->title }}"
                                class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                            <div
                                class="absolute top-4 left-4 bg-mcc-blue-900/80 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full">
                                {{ $article->published_at->format('M d, Y') }}
                            </div>
                        </div>
                        <div class="p-8 flex-grow flex flex-col">
                            <h4
                                class="text-xl font-bold text-mcc-slate-900 mb-4 line-clamp-2 min-h-[3.5rem] group-hover:text-mcc-blue-600 transition-colors">
                                {{ $article->title }}
                            </h4>
                            ...
                            <div class="mt-auto pt-6 border-t border-mcc-slate-50">
                                <a href="{{ route('articles.show', $article->slug) }}"
                                    class="inline-flex items-center text-xs font-bold text-mcc-blue-600 uppercase tracking-widest hover:text-mcc-blue-900 transition-colors">
                                    {{ __('Read Article') }}
                                    <svg class="ml-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
            ...
            <div class="bg-mcc-blue-900 rounded-[3rem] p-12 md:p-20 text-white relative overflow-hidden shadow-2xl" reveal>
                ...
                <div class="relative z-10 max-w-3xl space-y-8">
                    <h2 class="text-3xl md:text-5xl font-bold leading-tight">
                        {{ __('Ready to power your next major developmental leap?') }}
                    </h2>
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 pt-4">
                        <a href="{{ route('contact') }}"
                            class="btn-corporate bg-mcc-gold text-mcc-slate-900 hover:bg-white justify-center shadow-lg">
                            {{ __('Contact Our Global Offices') }}
                        </a>
                        <a href="{{ route('projects.index') }}"
                            class="btn-corporate bg-white/10 text-white border-white/20 hover:bg-white/20 justify-center">
