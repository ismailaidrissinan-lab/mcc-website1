    ...
    <div class="relative pt-32 pb-48 lg:pb-64 bg-mcc-slate-900 overflow-hidden">
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('images/mcc-logo.png') }}"
                alt="Shanghai Skyline" class="w-full h-full object-cover opacity-30 transform scale-105">
        ...
        <div class="container-wide relative z-10 text-center">
            <div
                ...
                <span
                    class="text-mcc-gold text-[10px] font-bold uppercase tracking-widest">{{ __('Corporate Governance') }}</span>
            </div>
            ...
            <div class="bg-white p-12 md:p-20 rounded-[3rem] shadow-2xl border border-mcc-slate-100">
                <div class="max-w-5xl mx-auto">
                    <div class="text-center mb-16">
                        <h2 class="text-3xl font-bold text-mcc-slate-900 mb-6">
                            {{ __('Stakeholder Transparency & Financial Integrity') }}</h2>
                        ...
                        @forelse($documents as $category => $docs)
                            <div class="space-y-8">
                                <div class="flex items-center gap-4 border-b border-mcc-slate-100 pb-4">
                                    <h3 class="text-lg font-bold text-mcc-blue-900 uppercase tracking-widest text-xs">
                                        {{ __($category) }}</h3>
                                    <span
                                        class="ml-auto bg-mcc-blue-50 text-mcc-blue-600 text-[10px] px-2 py-0.5 rounded-full font-bold">{{ $docs->count() }}</span>
                                </div>
                                <div class="space-y-6">
                                    @foreach($docs as $doc)
                                        <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank"
                                            ...
                                            <div
                                                class="w-10 h-10 bg-mcc-slate-50 rounded-lg flex items-center justify-center text-mcc-slate-400 group-hover:bg-mcc-blue-600 group-hover:text-white transition-colors">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <h4
                                                    class="text-mcc-slate-900 font-bold text-sm truncate group-hover:text-mcc-blue-600 transition-colors">
                                                    {{ $doc->title }}</h4>
                                                <p class="text-[10px] text-mcc-slate-400 mt-1 uppercase tracking-widest">
                                                    {{ $doc->published_at->format('M Y') }}</p>
                                            </div>
                                        </a>
                                    @endforeach
                                </div>
